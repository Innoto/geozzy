  ...
  /****************************
    paisaxesExplorer
   ****************************/
  aloxamentosExplorer = function() {
    var that = this;


    that.explorerclass = '.aloxamentosExplorer';
    that.mapOptions = false;
    that.resourceMap  = false;
    that.accommodationTypes = false;
    that.accommodationCategories = false;
    that.accommodationFacilities = false;
    that.accommodationServices = false;
    ...
    that.setInitialData = function( doneFunction ){
      ...
      that.accommodationTypes = new geozzy.collection.CategorytermCollection();
      that.accommodationTypes.setUrlByIdName('accommodationType');

      that.accommodationCategories = new geozzy.collection.CategorytermCollection();
      that.accommodationCategories.setUrlByIdName('accommodationCategory');

      that.accommodationFacilities = new geozzy.collection.CategorytermCollection();
      that.accommodationFacilities.setUrlByIdName('accommodationFacilities');

      that.accommodationServices = new geozzy.collection.CategorytermCollection();
      that.accommodationServices.setUrlByIdName('accommodationServices');

      that.zonaCategories = new geozzy.collection.CategorytermCollection();
      that.zonaCategories.setUrlByIdName('rextAppZonaType');

      // Multiple data fetch
      $.when( that.accommodationTypes.fetch(), that.accommodationCategories.fetch(), that.accommodationFacilities.fetch(), that.accommodationServices.fetch(), that.zonaCategories.fetch() ).done(function() {
        doneFunction();
      });
    }
    ...
    */
    ...
    that.setDisplays = function() {
      ...
      that.mapa = new geozzy.explorerComponents.mapView({
          ...
          chooseMarkerIcon: function( markerData ) {
            ...
            that.accommodationTypes.each( function(e){
    ...
    that.setFilters = function() {
      ...
      var filtroTipos = new geozzy.explorerComponents.filters.filterButtonsView(
        {
          ...
          containerClass: 'tipoAcommodation',
          //defaultOption: { icon: false, title: 'Todos os tipos', value:'*' },
          data: that.accommodationTypes
        }
      );
      ...
      that.explorer.addFilter( filtroCategoria );
      that.explorer.addFilter( filtroInstalaciones );
      that.explorer.addFilter( filtroServicios );
    ...
    that.interfaceFilters = function(){
      ...
      filterInterface += '<div class="filters_summary"><div class="filters_summary_aloxamentosInstalaciones"></div><div class="filters_summary_aloxamentosCategoria"></div><div class="filters_summary_aloxamentosServizos"></div><div class="filters_summary_aloxamentosPrezo"></div></div>';
