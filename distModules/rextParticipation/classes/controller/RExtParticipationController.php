...
class RExtParticipationController extends RExtController implements RExtInterface {

  public function __construct( $defRTypeCtrl = false ){
    if($defRTypeCtrl){
      parent::__construct( $defRTypeCtrl, new rextParticipation(), 'rExtParticipation_' );
    }
  ...
  public function resFormProcess( FormController $form, ResourceModel $resource ) {
    if( !$form->existErrors() ) {
      ...
      $rExtModel = new ParticipationModel( $valuesArray );
      if( $rExtModel === false ) {
   ...
   */
   public function getViewBlockInfo( $resId = false ) {
     ...
     if( $rExtViewBlockInfo['data'] ) {
       ...
       $rExtViewBlockInfo['template']['full']->setTpl( 'rExtViewBlock.tpl', 'rextParticipation' );
     }
